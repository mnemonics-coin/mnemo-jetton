...
import { buildTokenOnchainMetadataCell, JettonMinter, jettonMinterConfigToCell } from '../wrappers/JettonMinter';
...
describe('State init tests', () => {
  ...
  it('should mint max jetton walue', async () => {
    ...
    const res = await jettonMinter.sendMint(deployer.getSender(), deployer.address, maxValue, toNano(1));
